...
@click.option(
    ...
    default="configs/experiments/xgboost.yml",
...
def main(experiment_config_path, show_tree, log_level, scheduler_url, force_rerun):
    """Run the experiment pipeline with the specified configuration."""
    ...
    pipeline = ExperimentPipeline(experiment_config_path=experiment_config_path)
    ...
    click.echo(f"Running pipeline with config: {experiment_config_path}")
    gokart.build(
        task=pipeline,
